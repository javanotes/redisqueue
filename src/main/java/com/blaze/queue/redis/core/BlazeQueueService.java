...
@Service
@Qualifier("RMQ")
class BlazeQueueService implements QueueService{

	private static final Logger log = LoggerFactory.getLogger(BlazeQueueService.class);
	
	@Autowired
	private BlazeQueueContainer container;
	...
	{
		return BlazeQueueContainer.prepareKey(xchangeKey, routeKey);
	}
	...
	{
		return redisTemplate.boundListOps(prepareKey(xchangeKey, routeKey));
	}
	@Override
	...
	public QRecord getNext(String xchng, String route, long await, TimeUnit unit) throws TimeoutException {
		return container.fetchHead(xchng, route, await, unit);
	}
